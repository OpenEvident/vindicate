...
> from ground evidence per _Submit timing_);
...
combined via direct-edit — see _Hybrid features_ below). New features and new test cases go through
...
1. **Pick the operation** (see Decision rules), per the story's `layer`:
   ...
   - `hybrid` — run the `ui` and `api` ops independently (two separate `create`/`create_api` calls,
     ...
     into one spec case where the story actually needs a single mixed test (see _Hybrid features_).
...
- **Waits / synchronization** — put explicit waits **inside the page-object step**, never the spec:
  ...
  - **After submit / click that triggers async work** — pick **one wait from ground evidence**, inside the page-object step (never the spec):
    ...
    - **Not every login** needs `waitForResponse` — use it only when ground saw a submit XHR. Validation rejects `waitForResponse` without `observed_endpoints` or doc-copied paths. **Do not** use `waitForPageLoad` after submit. (See `ref-codegen-schema` → _Submit timing_.)
...
- **Auth: token-once, never per-test.** When the feature needs a login step, wire it as `create_api`'s `auth_setup` (worker-scoped fixtures — log in once per worker, share the token) — never a per-test login call. See `ref-api-codegen-schema` → _Auth setup_.
