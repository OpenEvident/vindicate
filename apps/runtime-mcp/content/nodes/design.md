...
so this is where the story is *finalized*: the AI proposes worthwhile additional scenarios, the user
...
2. **Propose additions (bounded).** Suggest a *small* set of high-value adjacent scenarios for the same feature (likely error/boundary/edge cases a tester would want), each clearly flagged in the panel as a **suggestion** vs the user's original ask. No exhaustive dump, no silent additions.
