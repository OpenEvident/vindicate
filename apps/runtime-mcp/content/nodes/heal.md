...
## Rules
- **Ask for credentials at the point of need.** If the forensic re-snapshot (`browser_read`) lands on a page behind auth, ask for the credential *then* via `vindicate_ask_user`, use it transiently to drive, and **never echo it back into chat**. Real runs read secrets from `.env`; anything shared in chat is flagged for rotation at close-out (see `escalate`/`audit`).
