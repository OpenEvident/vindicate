...
export function buildActionResult(input: {
  ...
  const roundTrip =
    !navigated &&
    urlTrail.length > 1 &&
    urlTrail.some((url) => url !== input.urlBefore);
  ...
  return {
    ...
    ...(page_change === "round_trip"
      ? {
          hint:
            "Navigated away and returned to the same URL — call browser_read before the next action. When authoring tests, pair this submit with waitForResponse on the endpoint seen in url_trail."
        }
      : {}),
