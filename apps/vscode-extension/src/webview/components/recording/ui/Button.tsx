...
const variantClasses: Record<ButtonVariant, string> = {
  ...
  danger:
    "bg-transparent text-vs-text-dim border-vs-border hover:text-tone-red hover:bg-tone-red/10 hover:border-tone-red/35"
};
...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    >
      {loading && (
        <Loader2 size={size === "sm" ? 12 : 14} className="animate-spin shrink-0" aria-hidden />
      )}
      {children}
    </button>
  )
